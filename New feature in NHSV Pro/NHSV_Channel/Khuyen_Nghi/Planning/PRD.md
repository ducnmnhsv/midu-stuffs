...
| ~~Q10~~ | ✅ **Đã xác nhận (2026-07-23, rà soát folder reorg):** `Design.html`/`Design_Variations.html`/`admin-demo.html` (nay ở `Design/`) thực tế đã cập nhật đầy đủ theo v1.2/v1.3 (Home entry point, tab order, bỏ Giá tại ngày KN, Segment auto, bỏ expiryDate, badge động, rolling 14 ngày, cảnh báo Excel, unique mã/ngày) — có annotation `(v1.2)`/`(v1.3)` trực tiếp trong file. `Spec.html` (nay ở `Archive/`, đã đối chiếu để viết `Specifications/Feature_Specification.md`) cũng đã cập nhật đầy đủ. Flag "vẫn phản ánh v1.1" ở entry gốc là **thông tin cũ, không còn đúng** — không có action gì thêm. | Resolved | PM | — |

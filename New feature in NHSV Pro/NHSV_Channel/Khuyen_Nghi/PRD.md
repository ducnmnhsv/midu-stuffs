...
Khuyến nghị kỹ thuật ngắn hạn (vài ngày → vài tuần) cho từng phiên giao dịch. Default load entries của ngày hôm nay; nếu Phòng PT chưa upload → empty state với icon đồng hồ + thông điệp "Phòng Phân tích thường cập nhật khuyến nghị kỹ thuật trong phiên sáng. Vui lòng quay lại sau."
...
Mọi screen của tab Khuyến nghị đều phải có footer disclaimer ở cuối list:
- Danh mục cơ bản & Kỹ thuật: "Khuyến nghị chỉ mang tính chất tham khảo. Quý khách cần cân nhắc trước mọi quyết định đầu tư."
- NHSV Rating: "Rating do Phòng QTRR cung cấp, cập nhật định kỳ hàng tháng."
...
Nút "+ Thêm khuyến nghị" → form: Mã CK (autocomplete), Segment, Giá tại ngày KN, Giá kỳ vọng (auto-calculate % tiềm năng live), Ngày KN, Luận điểm (textarea optional), Upload PDF (drag-drop optional).
...
Date picker để chọn ngày (default: hôm nay). Table: Mã CK | Vùng mua | Mục tiêu | Cắt lỗ | Upsize | PDF | Nguồn | Thao tác. Cột "Nguồn" có badge "MANUAL" (v1) hoặc "AUTO" (v2 — Vietstock automation).

Form thêm/sửa: Mã CK, Ngày, Vùng mua (from + to optional), Mục tiêu, Cắt lỗ, Upsize (optional), Luận điểm, PDF (optional).

Delete: HARD DELETE (khác với Danh mục cơ bản — dữ liệu kỹ thuật ngắn hạn, không cần giữ lâu).
...
|---|---|---|---|---|
| Q1 | Danh mục cơ bản — khuyến nghị có auto-expire không? Sau khi đạt target hoặc qua N tháng có tự archive? | Blocking — ảnh hưởng data model (cần thêm `expiry_date`?) | PM + Phòng PT | Trước BE-01 |
...
|---|---|---|
| Backend | 10 stories (BE-01 → BE-10) | ~3 sprint (2 tuần/sprint) |
| Mobile FE | 8 stories (MOB-01 → MOB-08) | ~2 sprint |
| Admin FE | 6 stories (ADM-01 → ADM-06) | ~2 sprint |
| QA | 6 stories (QA-01 → QA-06) | ~1 sprint |

Tổng: ~30 stories. Blocker lớn nhất: Q1 (data model expiry), Q5 (Excel format), và availability của market data service cho price enrichment.
...
- Phòng PT tạo entry Kỹ thuật hằng ngày trong < 3 phút
...
Document Status: 📋 Draft | For: IT Lead, Mobile FE, Admin FE, Phòng Phân tích, Phòng QTRR, Legal | Next Steps: Confirm Open Questions Q1–Q6 trước khi bắt đầu estimate chi tiết theo sprint

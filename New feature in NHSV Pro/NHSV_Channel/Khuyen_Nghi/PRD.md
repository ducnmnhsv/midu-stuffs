...
Table với filter Bluechip/Midcap + search mã + filter status (Active/Archived). Cột: Mã CK | Segment | Giá Kỳ vọng | Ngày KN | Report | Trạng thái | Thao tác. **(v1.2)** Bỏ cột "Giá KN" (không còn snapshot giá lúc khuyến nghị).

Nút "+ Thêm khuyến nghị" → form: Mã CK (autocomplete), Segment, Giá kỳ vọng (auto-calculate % tiềm năng live so với giá hiện tại), Ngày KN, Ngày hết hiệu lực (date picker, optional — để trống = không giới hạn thời gian), Luận điểm (textarea optional), **Report đính kèm** — chọn 1 trong 2 cách: (a) tìm & link 1 bài NH Research đã publish có tag mã này (autocomplete theo `GET /api/v1/nhResearch/articles?stockCode=X`, xem A-06), hoặc (b) Upload PDF riêng (drag-drop, dùng chung pipeline `/admin/api/upload/pdf`). **(v1.2)** Bỏ field "Giá tại ngày KN" khỏi form — % tiềm năng luôn tính live so với giá hiện tại, không cần điểm neo giá lúc tạo.
...
Date picker để chọn ngày (default: hôm nay) — **chọn được bất kỳ ngày trong quá khứ** (cập nhật 2026-07-10), không giới hạn hôm nay. **(v1.2)** Lưu ý: giới hạn "chỉ hiển thị 14 ngày gần nhất" chỉ áp dụng cho màn hình **mobile** (sub-tab Kỹ thuật, mục 4.1) — Admin vẫn quản lý/xem/sửa được entry của bất kỳ ngày nào trong quá khứ như v1.1, không bị giới hạn 14 ngày. Table hiển thị toàn bộ entry của ngày đã chọn, **gồm cả Nháp và đã Publish**. Table: Mã CK | Vùng mua | Mục tiêu | Cắt lỗ | Upsize | PDF | Nguồn | **Trạng thái** | Thao tác. Cột "Nguồn" có badge "MANUAL" (v1) hoặc "AUTO" (v2 — Vietstock automation).
...
|---|---|---|
| **(v1.2)** `GET /recommendations/portfolio/topPicks` | Preview Top pick cho card Home | ≤ 6 records, sort % tiềm năng giảm dần; response nhẹ (mã, %, segment) |
...
| `GET /recommendations/portfolio/{id}` | Detail | Trả `shortDesc` (luận điểm mốc hiện tại), `expiryDate`; **(v1.2)** thêm `reportHistory[]` (Danh sách báo cáo gần nhất — mỗi item: `recommendedDate`, `title`, `reportUrl`, `source`: `NH_RESEARCH` \| `UPLOAD`) |
...
| Q10 *(v1.2)* | `Design.html`/`Spec.html`/`admin-demo.html` hiện vẫn phản ánh model v1.1 (giá tại ngày KN, tab order cũ, Kỹ thuật chỉ hôm nay) — cần cập nhật lại theo v1.2 trước khi đưa cho IT estimate chi tiết | Doc consistency | PM | Trước khi giao IT estimate |
...
Document Status: 📋 Draft | For: IT Lead, Mobile FE, Admin FE, Phòng Phân tích, Phòng QTRR, Legal | Next Steps: v1.2 (2026-07-20) đổi model Home entry point + Cơ bản (mốc lịch sử) + Kỹ thuật (rolling 14 ngày) — cần confirm Q7–Q10 (mới) và Q3–Q6 (còn treo từ v1.1), đồng thời cập nhật lại `Design.html`/`Spec.html`/`admin-demo.html` theo v1.2 trước khi giao IT estimate chi tiết theo sprint

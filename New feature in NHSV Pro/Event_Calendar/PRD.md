...
- Hiển thị sự kiện cổ tức tiền mặt (cash dividend), cổ phiếu thưởng / cổ tức bằng cổ phiếu, và quyền mua phát hành thêm (rights issue)
...
- Loại sự kiện khác: AGM, kết quả kinh doanh
...
- `titleEvent` (nếu không null): text xám nhạt, font nhỏ, italic — đặt dưới pill, trước divider ngăn cách với phần Thông tin ngày
...
| RIGHTS_ISSUE | Amber (#fef3c7 / #b45309) | "Quyền mua phát hành thêm" |
...
| Field | CASH_DIVIDEND | STOCK_DIVIDEND | RIGHTS_ISSUE |
|---|---|---|---|
| Label tỷ lệ | "Tỷ lệ thực hiện" | "Tỷ lệ phát hành" | "Tỷ lệ phát hành thêm" |
| Giá trị `rateDisplay` | "2,000đ/CP" | "20%" | "10%" |
| Giá quyền mua | — | — | "15,000đ/CP" (từ `priceDisplay`) |
| Loại cổ tức | "Tiền mặt" | Theo `eventTypeLabel` | "Quyền mua phát hành thêm" |
| Sàn giao dịch | HOSE / HNX / Upcom | HOSE / HNX / Upcom | HOSE / HNX / Upcom |

> RIGHTS_ISSUE có thêm dòng **"Giá quyền mua"** trong detail screen, hiển thị `priceDisplay` (format "15,000đ/CP"). Ẩn dòng này nếu `price = null`.

**Link tài liệu:** Nếu `fileUrl` không null — hiển thị link text "📄 Xem tài liệu chính thức →" màu navy, đặt dưới Info card. Tap → mở URL trong in-app browser (WebView hoặc `Linking.openURL`). Ẩn hoàn toàn nếu `fileUrl = null`.
...
| channel_id | INT NOT NULL | 13/14/15/16 raw từ Vietstock |
| event_type | ENUM('CASH_DIVIDEND','STOCK_DIVIDEND','RIGHTS_ISSUE') NOT NULL | Derive từ channel_id |
| event_type_label | VARCHAR(100) NOT NULL | "Cổ tức tiền mặt" / "Cổ phiếu thưởng" / "Cổ tức bằng cổ phiếu" / "Quyền mua phát hành thêm" |
| gdkhq_date | DATE NOT NULL | KEY FIELD. Kiểu DATE (bỏ time component) |
| ndkcc_date | DATE | Nullable |
| main_rate | DECIMAL(10,4) NOT NULL | Raw decimal từ Vietstock.MainRate. VD: 0.1 (cash SSI), 1.0 (stock VIC), 0.1 (rights SSI) |
| rate_display | VARCHAR(50) NOT NULL | Formatted theo event_type: CASH_DIVIDEND → "1,000đ/CP" (0.1×10,000) · STOCK_DIVIDEND → "100%" (1.0×100) · RIGHTS_ISSUE → "10%" (0.1×100) |
| price | DECIMAL(15,2) | Nullable. Giá quyền mua — chỉ có ở ChannelID 16. VD: 15000.00 → hiển thị "15,000đ/CP" |
...
Theo dõi lịch sử call Vietstock theo từng mã — kiểm soát tần suất gọi lại.
...
| next_eligible_at | DATETIME NOT NULL | Tính theo `events_found`: xem bảng Skip Window bên dưới |
...
**Skip Window theo `events_found`:**

| events_found lần trước | next_eligible_at | Lý do |
|---|---|---|
| 0 (không có sự kiện) | `last_called_at + 6 MONTH` | Mã không active — skip dài để giảm API call |
| ≥ 1 (có sự kiện) | `last_called_at + 30 DAY` | Mã đang active — một mã có thể có nhiều quyền/sự kiện công bố gần nhau, cần sync thường xuyên hơn để không bỏ sót |

> **Known limitation:** Nếu một mã công bố sự kiện mới trong cửa sổ skip (6 tháng với mã không active, 30 ngày với mã active), sự kiện đó sẽ không được phát hiện cho đến lần sync tiếp theo. Đây là đánh đổi có chủ ý. Nếu cần bắt trường hợp này, cần thêm cơ chế manual trigger (ngoài scope v1).
...
| 16 | Additional share issuance (rights issue) | RIGHTS_ISSUE | Quyền mua phát hành thêm |

Chỉ giữ ChannelID 13/14/15/16. Bỏ các loại khác.
...
| MainRate | main_rate + rate_display | Lưu raw decimal vào main_rate. **Formula đã xác nhận từ data thực:** CASH_DIVIDEND → `round(MainRate × 10,000)` VNĐ/CP, format `"1,000đ/CP"` (SSI: 0.1 × 10,000 = 1,000). STOCK_DIVIDEND → `round(MainRate × 100)`%, format `"100%"` (VIC: 1.0 × 100 = 100%). RIGHTS_ISSUE → `round(MainRate × 100)`%, format `"10%"` (SSI: 0.1 × 100 = 10). |
| Price | price | Nullable DECIMAL. Chỉ có ở ChannelID 16. Giá quyền mua VNĐ/CP — lưu nguyên, hiển thị format `"15,000đ/CP"`. |
...
| FileUrl | file_url | URL PDF tài liệu chính thức từ sàn. Nullable. Hiển thị trong detail screen dưới dạng link "Xem tài liệu chính thức →". |
...
Từ response, chỉ giữ records có `ChannelID IN (13, 14, 15, 16)`. Parse `GDKHQDate` thành `DATE`. Format `rate_display` từ `MainRate`. Với ChannelID 16, lưu thêm `Price` vào cột `price`. Bỏ record có `GDKHQDate` null hoặc không parse được.
...
Sau khi call Vietstock thành công (dù có hay không có events), tính `next_eligible_at` theo số events tìm được:
- `events_found = 0` → `next_eligible_at = NOW() + 6 MONTH`
- `events_found ≥ 1` → `next_eligible_at = NOW() + 30 DAY`

Một mã có thể trả về nhiều events trong 1 lần call (VD: vừa có cổ tức tiền mặt vừa có quyền mua phát hành thêm). Skip ngắn hơn (30 ngày) để không bỏ sót sự kiện mới được công bố gần nhau.
...
```json
{
  ...
  "rateDisplay": "1,000đ/CP",
  "priceDisplay": null,
  "titleEvent": "Trả cổ tức năm 2024 bằng tiền, 1,000 đồng/CP",
  "fileUrl": "https://static2.vietstock.vn/vietstock/...",
  "isToday": true
}
```

- `ndkccDate`: nullable — FE ẩn dòng Ngày ĐKCC nếu giá trị này là null
- `priceDisplay`: nullable — chỉ có giá trị với `eventType = RIGHTS_ISSUE` (VD: `"15,000đ/CP"`). FE ẩn dòng "Giá quyền mua" nếu null.
- `titleEvent`: nullable — tiêu đề ngắn mô tả sự kiện từ Vietstock (VD: `"Thực hiện quyền mua cổ phiếu phát hành thêm, tỷ lệ 100:10, giá 15,000 đồng/CP"`). Hiển thị trong detail screen dưới hero card. FE ẩn nếu null.
- `fileUrl`: nullable — URL PDF tài liệu chính thức từ sàn (Vietstock.FileUrl). FE hiển thị link "Xem tài liệu chính thức →" mở in-app browser; ẩn nếu null.
...
Ví dụ rights issue: "SSI thực hiện Quyền mua phát hành thêm với tỷ lệ 10%, giá 15,000đ/CP. GDKHQ: 23/09/2024."

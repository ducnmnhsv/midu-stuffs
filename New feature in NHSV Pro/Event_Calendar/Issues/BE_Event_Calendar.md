...
| channel_id | INT | 13/14/15/16 raw từ Vietstock |
| event_type | ENUM('CASH_DIVIDEND','STOCK_DIVIDEND','RIGHTS_ISSUE') | Derive từ channel_id |
| event_type_label | VARCHAR(100) | "Cổ tức tiền mặt" / "Cổ phiếu thưởng" / "Cổ tức bằng cổ phiếu" / "Quyền mua phát hành thêm" |
...
| rate_display | VARCHAR(50) | Formatted theo event_type: CASH_DIVIDEND → "2,000đ/CP" · STOCK_DIVIDEND → "20%" · RIGHTS_ISSUE → "10%" |
| price | DECIMAL(15,2) | Nullable. Chỉ có ở ChannelID 16. Giá quyền mua VNĐ/CP (VD: 15000.00 → hiển thị "15,000đ/CP") |
...
| next_eligible_at | DATETIME | Tính theo events_found: `= 0` → `+6 MONTH`, `≥ 1` → `+30 DAY` |
...
3. Filter response: chỉ giữ `ChannelID IN (13, 14, 15, 16)`. Parse `GDKHQDate` → `DATE`. Với ChannelID 16, lưu `Price` vào cột `price`. Bỏ record có `GDKHQDate` null.
4. UPSERT vào `event_calendar`: `ON CONFLICT (vietstock_event_id) DO UPDATE SET ...`. Idempotent.
5. Cập nhật `stock_sync_tracker` sau mỗi call thành công. Tính `next_eligible_at` theo số events tìm được trong response sau khi filter:
   - `events_found = 0` → `next_eligible_at = NOW() + 6 MONTH`
   - `events_found ≥ 1` → `next_eligible_at = NOW() + 30 DAY`

   Lý do: một mã đang active có thể có nhiều sự kiện công bố gần nhau (VD: vừa cổ tức tiền mặt vừa quyền mua phát hành thêm) — skip ngắn hơn để không bỏ sót. Chỉ update khi HTTP 200 — nếu call lỗi không update tracker.
...
| 16 | RIGHTS_ISSUE | Quyền mua phát hành thêm |
...
- `RIGHTS_ISSUE` (ChannelID=16): `round(MainRate × 100)` = % → format `"{n}%"` + lưu `Price` riêng
  - SSI: MainRate=0.1 → 0.1 × 100 = **10%** · Price=15000 → **"15,000đ/CP"** ✅ (khớp Note: "tỷ lệ 100:10, giá 15,000 đồng/CP")
...
**Known limitation:** Nếu mã công bố sự kiện mới trong cửa sổ skip (6 tháng với mã không active, 30 ngày với mã active), sự kiện đó sẽ không được phát hiện cho đến lần sync tiếp theo. Đây là đánh đổi có chủ ý. Xem Open Question Q1 và Q3 trong PRD.
...
Response gồm tất cả fields của Event object + các fields chỉ có trong detail:
- `ndkccDate: string | null` — ngày ĐKCC
- `priceDisplay: string | null` — chỉ có giá trị với RIGHTS_ISSUE, VD: `"15,000đ/CP"`
- `titleEvent: string | null` — tiêu đề ngắn mô tả sự kiện từ Vietstock.Title_Event (VD: `"Trả cổ tức năm 2024 bằng tiền, 1,000 đồng/CP"`)
- `fileUrl: string | null` — URL PDF tài liệu chính thức từ Vietstock.FileUrl. Trả nguyên URL, FE tự mở in-app browser.
...
- Body: `{companyName} thực hiện {eventTypeLabel} với tỷ lệ {rateDisplay}. GDKHQ: {gdkhqDate}.` (với RIGHTS_ISSUE, nếu có price thêm `", giá {priceDisplay}"` sau rateDisplay)

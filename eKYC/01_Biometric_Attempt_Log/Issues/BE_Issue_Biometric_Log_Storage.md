...
Tạo changeset tạo bảng `ekyc_attempt_log` theo schema đầy đủ trong `Backend_Spec.md` Section 0.1 + 0.3. Bao gồm:
...
Copy nguyên cấu trúc field từ `Backend_Spec.md` Section 1 (đã viết sẵn đầy đủ). Chú ý:
...
Implement theo `Backend_Spec.md` Section 3:
...
Implement theo `Backend_Spec.md` Section 5 + Section 7.4 (BE Processing Logic). Method chính: `processAttemptLog(EKycAttemptLogRequest req, String identifierId, String phoneNo)`.
...
Tái dùng method `buildAttemptLog()` — xem `Backend_Spec.md` Section 2.3. Set cả `log.setVnptRawData(req.getVnptRawData())` (as-is).
...
Theo `Backend_Spec.md` Section 5 (đã có code mẫu đầy đủ).
...
Implement endpoint `POST /ekycs/attempt-log` theo `Backend_Spec.md` Section 5 (REST Controller) + Section 7 (API contract đầy đủ: request body, validation rules, response, error codes).
...
**Request DTO:** `EKycAttemptLogRequest.java` — map theo `Backend_Spec.md` Section 5 DTO + Section 7.2 (bao gồm `vnptRawData` và **field mới `sdkRawLogs`**).
...
Chi tiết đầy đủ: `Backend_Spec.md` Section 2.
...
Implement 3 endpoints theo `Backend_Spec.md` Section 4 (đã có response mẫu đầy đủ):
...
>
> `/list` với `accountStatus=NOT_APPROVED`: query gộp mọi customer mà lần thử gần nhất **không phải** `outcome = SUCCESS` — xem query logic đầy đủ ở `Backend_Spec.md` Section 4.1b.

---

## Acceptance Criteria

- [ ] Bảng `ekyc_attempt_log` được tạo qua Liquibase changeset — migration chạy thành công trên dev/staging, đúng schema `Backend_Spec.md` Section 0.1 (bao gồm cả `outcome`, `failure_step`, `phone_no`, `image_front_url`, `image_back_url`, `vnpt_raw_data`, `sdk_raw_logs`).
...
- **Image storage là sub-feature riêng (`05_Image_Storage/`):** `imageFrontBase64`/`imageBackBase64` → upload lên S3/MinIO cần `ImageStorageService` (xem `Backend_Spec.md` Section 8). Scope/retention policy đang chờ PM confirm — nếu chưa confirm, có thể tạm bỏ qua upload thật và để `image_front_url`/`image_back_url` = null, nhưng **KHÔNG bỏ qua việc lưu `vnpt_raw_data` và `sdk_raw_logs`** (2 cột raw JSON này độc lập với image storage, không cần chờ confirm).

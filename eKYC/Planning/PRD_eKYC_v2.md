...
9. Khách ký hợp đồng điện tử — *nằm ngoài phạm vi ghi log này*
...
- **Nếu hành trình không thành công** (lỗi rõ ràng ở một bước nào đó, hoặc khách bỏ dở không hoàn tất) **→ toàn bộ nhật ký các bước của lần đó sẽ bị xóa**, không giữ lại. Với trường hợp khách bỏ dở (không có lỗi rõ ràng, chỉ đơn giản là không tiếp tục), hệ thống sẽ chờ một khoảng thời gian (dự kiến 24–48 giờ không có hoạt động mới) trước khi coi là "bỏ dở" và xóa dữ liệu.
...
|---|---|---|---|
| 1 | Chờ bao lâu thì coi một hành trình là "khách bỏ dở" trước khi xóa dữ liệu? (đề xuất 24–48 giờ) | Dev Lead | Cần chốt trước khi triển khai |
| 2 | Nếu khách đóng ứng dụng và mở lại sau vài giờ để tiếp tục, hệ thống nhận diện đây là "cùng một hành trình" bằng cách nào? | Team ứng dụng di động + Dev Lead | Ảnh hưởng độ chính xác — nhận diện sai có thể xóa nhầm dữ liệu của khách vẫn đang thao tác |
| 3 | Một số bước tra cứu thông tin (danh sách ngân hàng, chi nhánh...) có thực sự cần ghi log không, hay chỉ gây nhiễu dữ liệu? | Dev Lead / Ops | Không chặn triển khai |
| 4 | Việc lưu vĩnh viễn dữ liệu nhạy cảm (CCCD, số điện thoại, IP) của các hồ sơ thành công có tuân thủ đầy đủ Nghị định 13/2023 về bảo vệ dữ liệu cá nhân không? | Compliance/Legal | Cần rà soát trước khi go-live |

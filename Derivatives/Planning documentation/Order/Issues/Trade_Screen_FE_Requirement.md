...
- **Actions:** Nút Buy (Long) (Max buy: n), Nút Sell (Short) (Max sell: n).
...
- **Actions:** Buy (Long) / Sell (Short) với Max buy / Max sell.
...
| **Order Availability Check** | GET | `/api/v1/derivatives/order/checkAvailability` | **Sức mua / sức bán:** query params `accountNumber`, `symbolCode`, optional `sellBuyType`. Response: `availableQuantity` (và `availableLiquidity`). Dùng để hiển thị Max buy / Max sell và validate quantity. |
...
| AC4 | Đặt lệnh Buy/Sell gọi `POST /api/v1/derivatives/order`; hiển thị message và orderNumber khi thành công. | Both |

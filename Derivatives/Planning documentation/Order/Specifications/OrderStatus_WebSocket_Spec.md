...
6. [FE Specification](#6-fe-specification) *(§6.0 Order Book — Open & All; §6.7 mapping WS ↔ history; §6.8 nút Sửa/Hủy)*
...
│  - On event: merge / upsert by orderNumber → update Order Book UI (Open & All) │
└─────────────────────────────────────────────────────────────────┘
```
...
| `evt_price` | `orderPrice` | String → Number |
...
Tên field và enum **`orderStatus`**, **`operation`**, **`orderPrice`**, **`orderQuantity`** phải **trùng** response `GET /rest/api/v1/derivatives/order/history` — không dùng `orderId` / `side` / `status` / `lifecycleAction` / `price` / `quantity` trên kênh TradeX này.
...
| **WebSocket** | Subscribe `order.status.{accountNumber}` khi vào Order Book (§6.1). Mỗi event: **cập nhật** dòng đã có hoặc **insert** dòng mới — **không** bắt buộc gọi lại `history` để thấy `orderNumber` mới (§6.2). |
...
| Thời điểm | Action |
|-----------|--------|
| Mount màn **Order Book** (sau khi đã gọi REST §2.3) | Subscribe `order.status.{accountNumber}` |
| Unmount màn Order Book | Unsubscribe |
...
| 10 | `evt_price` | String | `orderPrice` | Number/null | Parse to Number | Cùng tên REST history |
...
- [ ] Subscribe `order.status.{accountNumber}` khi mount **Order Book**
...
2. FE review §6 (Order Book Open & All; upsert WS; nút §6.8; socketCluster)  
...
5. Confirm **`orderPrice`** / **`orderQuantity`** trên WS (§5.2, §7.1)  

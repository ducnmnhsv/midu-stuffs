...
**Lotte reference:** [DRACC-038 — `dr-create-sub-account`](../../../Documentation/Lotte_DR_API_Specs.md) §2.1.9; ACC-032 — `status-payment-area-acc`
...
- Error / response format: [`Knowledge/TradeX/API Standards/tradex-api-conventions.md`](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md)
- Production FPT integration pattern: [`Knowledge/TradeX/System/ekyc-flow-upload-lotte-fpt-econtract.md`](../../../../Knowledge/TradeX/System/ekyc-flow-upload-lotte-fpt-econtract.md) (polling-based, không webhook)

---

## 1. Tóm tắt điều hành

Khách hàng cá nhân đã có **tài khoản chứng khoán cơ sở (TKCK) tại NHSV** có thể **tự phục vụ mở tiểu khoản phái sinh (sub `80`)** ngay trên app NHSV Pro. Hệ thống chặn các trường hợp không đủ điều kiện từ cả **FE (pre-flight)** lẫn **BE (authoritative)**; với khách đủ điều kiện, BE **khởi tạo hợp đồng điện tử** qua **FPT eContract** theo template `DERIVATIVES` (ký ảnh, TTL 90 ngày), **poll trạng thái FPT** cho tới khi KH ký xong, rồi gọi **Lotte DRACC-038** để gửi yêu cầu tạo sub `80`. Sau khi DRACC-038 thành công, BE phải gọi **ACC-032** kiểm tra lại trạng thái account; chỉ khi `derivatives_status` trả về `Y...` thì mới cập nhật DB hoàn tất và **push OneSignal** + **gửi email** thông báo thành công.
...
| ACC-032 | API Lotte tra cứu trạng thái account — `status-payment-area-acc`; dùng xác nhận sub phái sinh đã được duyệt sau DRACC-038 |
...
| Gọi ACC-032 xác nhận `derivatives_status = Y...` trước khi gửi push/email thành công | Thông báo SMS |
| **Batch job poll FPT status (Job A)** và **retry Lotte + check ACC-032 (Job B)** chạy 24/7 mỗi 5 phút | Hệ thống scheduler chung (dùng cơ chế cron sẵn có của TradeX) |
...
sequenceDiagram
    ...
    participant LT as Lotte DRACC-038 / ACC-032
    ...
    LB->>LT: checkDerivativesAccountStatus (ACC-032)
    LT-->>LB: derivatives_status
    alt derivatives_status = Y...
        LB->>NOTI: Outbox: push + email (locale từ DB)
        NOTI->>KH: Push + Email §12
    else derivatives_status = N... hoặc ACC-032 lỗi
        LB->>LB: Giữ trạng thái đang xử lý; Job B kiểm tra lại sau 5 phút
    end
```
...
| `deviceUniqueId` | String | **Y** | Device ID / MAC; audit trail — FE bắt buộc gửi (per [tradex-api-conventions §Auto-Populated](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md)) |

**Auto-populated fields (FE KHÔNG cần gửi):** `sourceIp` (rest-proxy điền từ request IP), `userId` / `name` / `identifierNumber` (lotte-bridge lấy từ JWT `userData`), `mdm_tp` (derive từ platform). Xem [tradex-api-conventions §1.1](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md).
...
- HTTP status phản ánh kết quả — **NO** `success: true/false`, **NO** `code: "0000"` trong body (per [tradex-api-conventions §Response Format Standards](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md)).
...
| `lotte_dracc_success_at` | Timestamp | Thời điểm DRACC-038 trả `0000` + `success=true`; dùng để phân biệt đã gửi yêu cầu mở sub nhưng chưa ACC-032 xác nhận |
| `account_derivatives_status` | String | Giá trị raw từ ACC-032 `data_list[0].derivatives_status`, ví dụ `N.Chưa đăng ký/Đã xóa đăng ký` hoặc `Y.Đã đăng ký` |
| `account_status_checked_at` | Timestamp | Lần gần nhất gọi ACC-032 |
| `account_status_error` | String | Lỗi kỹ thuật khi gọi/parse ACC-032 nếu có |
...
- **Production `ekyc-admin` hiện tại** dùng **polling 15–30 phút** (xem [TradeX Knowledge — ekyc-flow-upload-lotte-fpt-econtract](../../../../Knowledge/TradeX/System/ekyc-flow-upload-lotte-fpt-econtract.md)). Webhook "có thể có" nhưng chưa được xác minh trong code.
...
|-------------------------------------|-------------------------|-------------|
| `recipientStatus = signed` (với `recipientContactInfo.id = p_002_r_001` — KH) | `SIGNED` | **Trigger** gọi Lotte DRACC-038 (§11), sau đó ACC-032 xác nhận trạng thái sub + Job C download PDF |
...
- **Single handler idempotent:** Polling, webhook (nếu bật), và Job A **đều gọi chung một method** `handleEnvelopeStatusUpdate(envelopeId, newStatus, source)`. Method này:
  ...
  - CAS (compare-and-set) `lotte_status` từ `NOT_YET/FAILED` → `CALLING` trước khi gọi DRACC-038. Nếu DRACC-038 đã success nhưng ACC-032 còn trả `N...`, tiếp tục giữ `CALLING` để Job B check lại.
...
| Verify method | `checkDerivativesAccountStatus(registration: DerivativesSubRegistration): AccountStatusResponse` |
| Verify URL (ACC-032) | `{LOTTE_API}/tsol/apikey/tuxsvc/account/status-payment-area-acc` |
| Method | `POST` |
...
**Mapping TradeX → Lotte (per [tradex-api-conventions §Auto-Populated](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md)):**
...
Khi DRACC-038 trả `error_code == "0000"` **và** `success == true`, BE **chưa được coi là mở sub hoàn tất ngay**. BE cần:

1. Lưu kết quả DRACC-038 thành công, set `lotte_status = CALLING` hoặc trạng thái đang xử lý tương đương.
1. Gọi ACC-032 `/tsol/apikey/tuxsvc/account/status-payment-area-acc` để kiểm tra lại trạng thái account.
1. Chỉ khi ACC-032 trả `derivatives_status` bắt đầu bằng `Y` thì mới set `lotte_status = SUCCESS`, `lotte_error_code = "0000"`, cập nhật DB hoàn tất và trigger **§12 thông báo** (OneSignal + Email) qua outbox pattern.
1. Nếu ACC-032 trả `derivatives_status` bắt đầu bằng `N`, giữ record ở trạng thái chưa hoàn tất/đang xử lý; **không gửi push/email**. Job B sẽ gọi lại ACC-032 mỗi 5 phút để lấy trạng thái mới nhất.
1. Nếu ACC-032 lỗi HTTP/timeout/parse, không đổi sang `SUCCESS`; ghi nhận lỗi kỹ thuật và chờ Job B lần kế tiếp.

Sample ACC-032 response:
...
```json
{
  ...
  "data_list": [
    {
      "status_vsd": "2.VSD duyệt",
      "basic_stock_status": "Y.Đã đăng ký",
      "business_status": "Y.Đã đăng ký",
      "government_status": "Y.Đã đăng ký",
      "derivatives_status": "N.Chưa đăng ký/Đã xóa đăng ký"
    }
  ]
}
```

ACC-032 decision rule:

| Điều kiện ACC-032 | DB update | Notification |
|-------------------|-----------|--------------|
| `error_code = "0000"`, `success = true`, `data_list[0].derivatives_status` bắt đầu bằng `Y` | `lotte_status = SUCCESS`, lưu trạng thái account mới nhất | Gửi push + email thành công |
| `error_code = "0000"`, `success = true`, `data_list[0].derivatives_status` bắt đầu bằng `N` | Giữ trạng thái đang xử lý/chưa hoàn tất | Không gửi; Job B check lại sau 5 phút |
| ACC-032 lỗi HTTP/timeout/parse hoặc `data_list` rỗng | Giữ trạng thái hiện tại, lưu lỗi kỹ thuật | Không gửi; Job B check lại sau 5 phút |

Sau khi ACC-032 xác nhận `derivatives_status = Y...`, BE refresh cache `accountSubs` cho KH (để lần login tiếp theo thấy sub 80).
...
**Quan trọng — khoảng trống đặc tả (xem §2.1.9 DRACC-038 trong [Lotte_DR.md](../../../Documentation/Lotte_DR_API_Specs.md)):** `data_list` **rỗng** trong sample Lotte. **Không** giả định `sub_no` mới trả trong payload. Nguồn xác nhận MVP là ACC-032 field `derivatives_status`; nếu nhiều lần Job B vẫn trả `N` tới retry cap, chuyển `lotte_status = NEEDS_VERIFICATION` để Ops xác minh.

**Error pass-through (theo [tradex-api-conventions §Pass-Through Core Errors](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md)):** Nếu luồng register trả lỗi Lotte về FE (trường hợp sync-call hiếm gặp), dùng format `SUB_ACCOUNT_REGISTER_LOTTE_{error_code}` — ví dụ `SUB_ACCOUNT_REGISTER_LOTTE_1005` với `message = error_desc` pass-through. Thông thường, Lotte call là async (qua Job B), nên FE không nhìn thấy lỗi này trực tiếp.

---

## 12. Bước 6 — Thông báo thành công

Trigger chỉ khi `lotte_status` chuyển sang `SUCCESS` **sau khi ACC-032 xác nhận `derivatives_status = Y...`**. DRACC-038 trả `error_code = "0000"` chỉ chứng minh yêu cầu mở sub đã được gửi/xử lý phía Lotte, chưa đủ điều kiện gửi thông báo cho khách hàng.
...
- **Outbox pattern:** ghi event `NOTIFICATION_PENDING` trong cùng transaction với update `lotte_status = SUCCESS` sau ACC-032 `derivatives_status = Y...`; worker publish OneSignal + Email → không double-send nếu service chết giữa chừng.
...
- Job B retry Lotte + kiểm tra ACC-032: **mỗi 5 phút**, exponential backoff với DRACC failure; ACC-032 `N...` thì check lại ở lần chạy sau.
...
### 13.2 Job B — Lotte provisioning retry & ACC-032 verification

| Mục | Giá trị |
|-----|---------|
| Mục tiêu | Retry DRACC-038 cho các HĐ đã ký mà Lotte call chưa thành công; đồng thời gọi ACC-032 để xác nhận sub đã thật sự được duyệt trước khi gửi notification |
| Tần suất | **5 phút** / lần, 24/7 |
| Input query | `WHERE contract_status = 'SIGNED' AND lotte_status IN ('NOT_YET', 'CALLING', 'FAILED') AND lotte_retry_count < 48 AND (lotte_last_attempt_at IS NULL OR lotte_last_attempt_at < NOW() - INTERVAL '5 min')` |
| Action mỗi record | Nếu chưa gọi/DRACC failed: gọi `provisionDerivativesSubAccount` (§11.1). Nếu DRACC đã success nhưng ACC-032 chưa trả `Y`: gọi `checkDerivativesAccountStatus` (ACC-032). Tăng `lotte_retry_count`, cập nhật `lotte_last_attempt_at` |
| Dừng retry | Khi `lotte_retry_count >= 48` (≈ 4 giờ) → chuyển `lotte_status = NEEDS_VERIFICATION` và alert Ops; không auto-retry thêm |
| Thành công | Chỉ khi ACC-032 trả `derivatives_status = Y...` → `lotte_status = SUCCESS`, trigger notification + Job C (download PDF) |
| ACC-032 trả `N...` | Giữ trạng thái đang xử lý (`CALLING` hoặc tương đương), không gửi notification; job sau 5 phút gọi lại ACC-032 |
| ACC-032 lỗi | Giữ trạng thái hiện tại, lưu lỗi kỹ thuật; job sau 5 phút thử lại |
| Idempotency | CAS `lotte_status = CALLING` trước khi gọi DRACC-038; sau DRACC/ACC-032 response → chỉ set `SUCCESS` khi ACC-032 `Y...`, set/giữ `CALLING` khi ACC-032 `N...`, set `FAILED` khi DRACC lỗi |
...
LOTTE_CALLING
  │
  ├─(DRACC 0000 + success=true)──▶ ACC032_CHECKING
  │                                  │
  │                                  ├─(derivatives_status = Y...)──▶ PROVISIONED ──▶ NOTIFIED (terminal success)
  │                                  │
  │                                  └─(derivatives_status = N... / ACC-032 lỗi)──▶ LOTTE_CALLING
  │
  ├─(1005 / 5xx)───────────▶ LOTTE_FAILED
  │                              │ (Job B retry mỗi 5 phút, cap 48 lần)
  │                              ▼
  │                         (nếu retry + ACC-032 Y) → PROVISIONED
  │                              │ (cap exhausted)
  │                              ▼
...
| Observability | Metric: count by state, Lotte latency p95, ACC-032 latency/fail rate, FPT login/recipient/cancel latency p95, **Job A/B/C success ratio**, Minio upload p95. Alert: `LOTTE_FAILED` > 5/giờ, Job B backlog > 100 record, ACC-032 fail rate > 10%, Job C fail rate > 10%, FPT login fail > 3 lần liên tiếp |
...
| I18n | Copy text tiếng Việt chuẩn (Pháp chế duyệt); admin UI hỗ trợ tiếng Việt + tiếng Anh cho label; error body không embed i18n — FE map `code` → i18n bundle (theo [tradex-api-conventions](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md)) |
...
| Lotte DRACC-038 availability | KH không thể gửi yêu cầu mở sub dù đã ký | Job B retry 5 phút cap 48 lần, circuit breaker, KH thấy “Đang xử lý” |
| Lotte ACC-032 availability / trạng thái `derivatives_status` cập nhật chậm | DRACC-038 đã success nhưng chưa đủ điều kiện xác nhận mở sub thành công | Job B gọi ACC-032 mỗi 5 phút với các record chưa SUCCESS; chỉ gửi push/email khi ACC-032 trả `derivatives_status = Y...` |
...
- [ ] Response body không chứa field `success`; mọi lỗi nghiệp vụ trả HTTP status code phù hợp (400 / 409 / 422) với body `{ code, params | message }` theo [tradex-api-conventions](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md).
...
**Lotte DRACC-038:**
- [ ] Lotte DRACC-038 trả `0000` + `success=true` nhưng ACC-032 chưa trả `derivatives_status = Y...` → **không** gửi OneSignal/email.
- [ ] Sau DRACC-038 success, BE gọi ACC-032 `/tsol/apikey/tuxsvc/account/status-payment-area-acc` để kiểm tra `derivatives_status`.
- [ ] ACC-032 trả `derivatives_status = Y...` → `lotte_status = SUCCESS`, OneSignal + email gửi **đúng một lần**.
- [ ] ACC-032 trả `derivatives_status = N...` → giữ trạng thái đang xử lý/chưa hoàn tất, Job B gọi lại sau 5 phút.
- [ ] ACC-032 lỗi HTTP/timeout/parse → không đổi `lotte_status = SUCCESS`; Job B thử lại ở lần chạy sau.
...
- [ ] Job B gọi ACC-032 mỗi 5 phút với các record đã DRACC success nhưng `derivatives_status` vẫn là `N...`/chưa xác nhận.
...
**Notifications:**
- [ ] KH nhận 1 push + 1 email chỉ sau khi ACC-032 xác nhận `derivatives_status = Y...`.
...
| Tài liệu | Mục đích |
|----------|----------|
| [PRD v1.0](../Archive/PRD_Open_Derivatives_Sub_Account_Online.md) | Framework planning — scope, stakeholder, state tổng quan |
| [Lotte_DR.md §2.1.9 DRACC-038](../../../Documentation/Lotte_DR_API_Specs.md) | Spec Lotte API — `dr-create-sub-account` |
| `/Users/ducnguyen/Downloads/FPT.eContract_API_POC_102023_v3.0 (1).pdf` | FPT eContract API POC v3.0 (10/2023) — revalidation source |
| [TradeX Knowledge — ekyc-flow-upload-lotte-fpt-econtract](../../../../Knowledge/TradeX/System/ekyc-flow-upload-lotte-fpt-econtract.md) | Pattern cron / polling / idempotency production tham khảo |
| [TradeX Knowledge — ekyc-signature-from-fpt-econtract](../../../../Knowledge/TradeX/System/ekyc-signature-from-fpt-econtract.md) | Pattern retrieve signature từ FPT + Lotte update flow |
| [TradeX Knowledge — tradex-api-conventions](../../../../Knowledge/TradeX/API%20Standards/tradex-api-conventions.md) | Validation strategy, error format, auto-populated fields, no `success` field |
...
8. Định nghĩa **runbook Ops** cho trạng thái `NEEDS_VERIFICATION` (khi retry Lotte/ACC-032 đã cap) và luồng huỷ envelope 3.2.6.

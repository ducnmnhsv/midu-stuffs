...
**Lotte reference:** [DRACC-038 — `dr-create-sub-account`](../../../Documentation/%5BAPI%20specs%5DLotte_DR.md) §2.1.9; ACC-032 — `status-payment-area-acc`
...
**Quan trọng — khoảng trống đặc tả (xem §2.1.9 DRACC-038 trong [Lotte_DR.md](../../../Documentation/%5BAPI%20specs%5DLotte_DR.md)):** `data_list` **rỗng** trong sample Lotte. **Không** giả định `sub_no` mới trả trong payload. Nguồn xác nhận MVP là ACC-032 field `derivatives_status`; nếu nhiều lần Job B vẫn trả `N` tới retry cap, chuyển `lotte_status = NEEDS_VERIFICATION` để Ops xác minh.

...
## fix error: jhipster CreateProcess error=193, %1 is not a valid Win32 application
```
mvn dependency:purge-local-repository -DmanualInclude=com.github.eirslett:node
mvn clean package
```

## Generate model, table from jdl file
```
jhipster import-jdl <path-to-file>
```

## fix errror: cannot generate table
Reference: 
...
rename database or drop table, then run :
```
mvnw
```
...
To build the final jar and optimize the eKyc_admin application for production, run:

...
export const getEntitiesFilter = createAsyncThunk('userManagement/getFiler',
...
export const updateUser = createAsyncThunk(
  ...
  async (user: IUser) => {
    user.copyMarketLeaderDetailsDTO = null;
    const photo = user.photo;
    user.photo = null;
    const result = (await axios.put<IUser>(adminUrl, user));
    if(photo) {
      user.photo = photo;
      await axios.post<IUser>(adminUrl + "/upload", user, {
        headers: {
...
export const UserManagementSlice = createSlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addMatcher(isFulfilled(getUsers, getEntitiesFilter), (state, action) => {
        state.loading = false;
      ...
      .addMatcher(isPending(getUsers, getUser, getEntitiesFilter), state => {
      ...
      .addMatcher(isRejected(getUsers, getUser, getRoles, updateUser, updateStatus, getEntitiesFilter), (state, action) => {
        state.loading = false;
        state.updating = false;
        state.updateSuccess = false;

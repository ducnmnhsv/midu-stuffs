...
export const saveAccountSettings: (account: any) => AppThunk = account => async dispatch => {
  ...
  if(account.photo){
    await dispatch(uploadImage(account));
  }
  dispatch(getSession());
};
...
export const uploadImage = createAsyncThunk('settings/uploadImage', async (account: any) => axios.post<any>("api/admin/users/upload", account, {
  headers: {

...
public class UserMapper {
    ...
    public List<User> userDTOsToUsers(List<AdminUserDTO> userDTOs) {
        return userDTOs.stream().filter(Objects::nonNull).map(UserMapper::userDTOToUser).collect(Collectors.toList());
    }

    public static User userDTOToUser(AdminUserDTO userDTO) {
        if (userDTO == null) {
            return null;
        } else {
            User user = new User();
            ...
            Set<Authority> authorities = UserMapper.authoritiesFromStrings(userDTO.getAuthorities());

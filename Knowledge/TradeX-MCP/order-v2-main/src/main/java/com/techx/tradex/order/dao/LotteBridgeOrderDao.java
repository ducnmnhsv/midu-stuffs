...
import com.techx.tradex.order.model.request.LotteOrderPlaceRequest;
...
import com.techx.tradex.order.model.response.LotteOrderPlaceResponse;
...
public class LotteBridgeOrderDao implements BridgeOrderDao {
    private static final Logger log = LoggerFactory.getLogger(LotteBridgeOrderDao.class);
    ...
    @Autowired
    public LotteBridgeOrderDao(AppConf appConf, RequestSender requestSender,
    ...
    public void placeRealOrder(StopOrder stopOrder, SymbolInfo symbolInfo) {
        ...
        try {
            ...
            LotteOrderPlaceRequest request = LotteOrderPlaceRequest.fromStopOrder(stopOrder, symbolInfo,appConf.isEnableLotteBridge());
            log.info("Request PlaceRealOrder from stopOrder {}", request);
            Message<?> message;
            if(appConf.isEnableLotteBridge()) {
                message = requestSender.sendAsyncRequest(appConf.getTopics().getLotteBridge(), appConf.getUri().getLotteEquityOrder(), appConf.getClusterId(), request).get();
            } else {
                if (SymbolTypeEnum.FUTURES.equals(symbolInfo.getType())) { //place derivatives order
                    message = requestSender.sendAsyncRequest(appConf.getTopics().getTuxedo(), appConf.getUri().getTuxDrOrder(), appConf.getClusterId(), request).get();
                } else { //place eqt order
                    message = requestSender.sendAsyncRequest(appConf.getTopics().getTuxedo(), appConf.getUri().getTuxEquityOrder(), appConf.getClusterId(), request).get();
                }
            }
            ...
            try {
                ...
                if(appConf.isEnableLotteBridge()){
                    stopOrder.setFailReason(e.getCode());
                } else {
    ...
    public ProfitLossOrder placeRealProfitLossOrderSync(ProfitLossOrder plOrder, SymbolInfo symbolInfo) {
        ...
        try {
            ...
            LotteOrderPlaceRequest request = LotteOrderPlaceRequest.fromProfitLossOrder(plOrder, symbolInfo);
            ...
            log.info("placeRealOrder response from bos: {}", message);
            try {
                LotteOrderPlaceResponse response = message.getResponse(objectMapper, new TypeReference<>() {
                });
                log.info("mas-rest-bridge order result: {}", response);
                plOrder.setStatus(ProfitLossOrderStatusEnum.PENDING);
                plOrder.setOrderNumber(response.getOrderNumber());
                plOrder.setOrderGroupNumber(null);
            } catch (GeneralException e) {

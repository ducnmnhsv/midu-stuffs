# [BE] Market Quote Fields Enhancement — Basis (`bs`) + Accumulated Active Volume (`asv` / `abv`)
...
| KL mua chủ động lũy kế | Không có | BE tính sẵn, trả field `abv` |
| KL bán chủ động lũy kế | Không có | BE tính sẵn, trả field `asv` |
| Reset đầu phiên | — | `asv` / `abv` reset về 0 khi sang phiên mới |
...
- **Khối lượng chủ động:** Mỗi lệnh khớp đã mang thông tin bên chủ động (mua hay bán). BE cộng dồn khối lượng từng lệnh khớp vào đúng bộ đếm (mua chủ động / bán chủ động) theo từng phiên giao dịch, và đặt lại về 0 khi bắt đầu phiên mới.
...
3. Field `asv` (bán chủ động) và `abv` (mua chủ động) xuất hiện trong response quote.
4. Giá trị `asv` / `abv` khớp với tổng KL khớp theo từng chiều chủ động trong phiên.
5. `asv` / `abv` reset đúng tại đầu mỗi phiên giao dịch.
...
### Naming reconciliation — đọc trước khi implement

Các field này **đã được đặt tên** trong convention TradeX (`TradeX Knowledge/System/symbolinfo-api-fields-guide.md`). Issue này là **implement field đã spec sẵn**, không phải thiết kế field mới.

| Khái niệm | Tên dùng (convention) | Full name | Ý nghĩa |
|-----------|----------------------|-----------|---------|
| Basis | `bs` | basis | Giá HĐTL − Giá chỉ số cơ sở (điểm) |
| Bán chủ động lũy kế | `asv` | accumulatedSellVolume | Tổng KL khớp với bên **bán** chủ động trong phiên |
| Mua chủ động lũy kế | `abv` | accumulatedBidVolume | Tổng KL khớp với bên **mua** chủ động trong phiên |
| Mã chỉ số cơ sở | `bc` | baseCode | Mã chỉ số cơ sở, vd `"VN30"` |

> ⚠️ **Lưu ý naming:** Brief gốc của FE gọi field "mua chủ động" là `asb`. Convention TradeX đã dùng **`abv`** (accumulatedBidVolume) cho chính field này. **Dùng `abv`**, không tạo field `asb` mới (sẽ trùng lặp). FE cần được thông báo mapping `asb` → `abv` để hai phía không lệch nhau.
...
| `mb` | Ý nghĩa | Cộng vào |
|------|---------|----------|
| `ASK` | Giá khớp từ bên bán → **bán chủ động** (aggressive sell) | `asv` (accumulatedSellVolume) |
| `BID` | Giá khớp từ bên mua → **mua chủ động** (aggressive buy) | `abv` (accumulatedBidVolume) |

> ⚠️ Mapping trên theo convention authoritative (`symbolinfo-api-fields-guide.md`): **`ASK` = bên bán chủ động, `BID` = bên mua chủ động**. Brief gốc của FE ghi ngược (`ASK → mua chủ động`); **dùng định nghĩa trong Knowledge guide**, không theo brief. Cần FE confirm để hai phía không lệch chiều.
...
| Realtime | `realtime-v2` `QuoteUpdateHandler` / `QuoteService.updateQuote()` | Cộng dồn `asv`/`abv` theo phiên từ `mb`+`mv`; tính `bs` từ HĐTL vs chỉ số `bc`; ghi Redis SymbolInfo |
| Realtime | `realtime-v2` `MarketStatusService` (reset path) | Reset `asv`/`abv` = 0 tại đầu phiên |
...
| WS publish | `ws-v2` `parser.js` `convertDataPublishV2Quote()` | Thêm short-key `bs`, `asv`, `abv` vào payload |
| REST | `market-query-v2` (`/symbol/{code}/quote`) | Đảm bảo nhóm field phái sinh gồm `bs`, `asv`, `abv` được trả |
...
### Clarify-questions cho BE (cần chốt trong grooming) — 7 mục

1. **`asb` vs `abv`:** Brief FE gọi field mua chủ động là `asb`; convention dùng `abv`. → Đề xuất dùng `abv`, thông báo FE mapping. **Cần chốt.**
2. **Chiều aggressor (`mb` → asv/abv):** Brief FE ghi `ASK → mua chủ động`; convention guide ghi ngược (`ASK = bán chủ động`, `BID = mua chủ động`). Doc này theo convention. **FE cần confirm để không cộng lệch chiều.**
3. **Channel `market.quote.dr`:** Brief nhắc kênh `market.quote.dr`. Knowledge chỉ document `market.quote.{symbol}` (vd `41I1G6000`). Suffix `.dr` không có trong Knowledge — có thể là typo hoặc suffix instance/env. **Cần xác nhận tên kênh chính xác.**
4. **Quy tắc prefix `41I1`/`41I2` → VN30/VN100:** Là input từ FE spec, chưa verify với symbol scheme của Lotte hay code live. BE resolve underlying bằng prefix hay bằng field metadata sẵn có? **Cần xác nhận.**
5. **VN100 futures:** Knowledge chỉ document HĐTL VN30. Sự tồn tại của HĐTL VN100 cần xác nhận.
6. **Giá tham chiếu Basis trong ATO/ATC:** Đề xuất dùng `ep`, nhưng là quyết định thiết kế của BE.
7. **REST hiện có expose nhóm field phái sinh chưa:** Cần xác nhận `/symbol/{code}/quote` hiện đã trả nhóm field phái sinh để bổ sung `bs`/`asv`/`abv`.
...
**Document Status:** 🟡 Draft — chờ validator review & BE grooming
**For:** Backend team (realtime-v2, ws-v2, market-query-v2), team-lead, FE (naming alignment)
**Next Steps:** BE trả lời 7 clarify-questions → chốt naming `abv` + chiều aggressor → implement tại `realtime-v2` + `ws-v2/parser.js` → verify REST + socket payload

...
- **Không lưu lịch sử cho user** — thông báo đã gửi không xuất hiện trong app; user bỏ lỡ push là mất luôn nội dung (webhook lưu notification NHMTS-870/871 đã có spec nhưng chưa deploy).
...
- 5 loại thông báo: Khuyến mãi, Tin tức, Báo cáo ngày, Nhắc nhở, Mặc định — loại quyết định field nhập (chỉ Khuyến mãi/Tin tức có HTML content, ảnh, deeplink)
...
| 3 | API gửi thiết kế làm **nền chung** — có sẵn tham số phân loại đối tượng nhận (ALL/SEGMENT/USER_LIST) dù UI Phase 1 chỉ dùng ALL | 2 feature khác đang chờ cùng 1 notification service (NHSV Channel, Event Calendar) — tránh xây 3 API trùng nhau |
...
| Webhook NHMTS-870/871 deploy sau này thành kênh gửi thứ 2 | Cần cơ chế chống trùng theo mã message OneSignal | Đã thiết kế sẵn cột unique `onesignal_message_id` |
